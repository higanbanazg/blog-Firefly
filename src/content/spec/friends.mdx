...
{/* 这里是"本站信息"卡片的数据，供来访者复制去添加本站友链。
    原值整块是主题作者 cuteleaf 的站点和邮箱，已替换成本站的。
    申请渠道原来是明文邮箱（会被爬虫抓去发垃圾邮件），已换成 Telegram：
    telegram 是展示用的 @handle，telegramUrl 是下面 Step 2 里点击跳转的地址，
    两者要一起改。
    avatar 原本指 github.com/higanbanazg.png（GitHub 头像），已改成本站图标；
    这张图同时是浏览器标签页图标，源图只有 128x85，192 那档是放大的。 */}

export const site = {
  name: "higanbana",
  desc: "higanbana的随记分享",
  url: "https://blog.ipfox.cc",
  avatar: "https://blog.ipfox.cc/favicon/higanbana-192.png",
  telegram: "@higanbanacn",
  telegramUrl: "https://t.me/higanbanacn",
};
      ...
      <div class="p-5 sm:p-6 flex flex-col flex-1">
        ...
        <div class="space-y-0 flex-1">
          ...
          <div class="flex gap-3.5">
            ...
            <div class="pb-4">
              <p class="font-semibold text-sm mb-1">发送申请至telegram：<a href={site.telegramUrl} target="_blank" rel="noopener noreferrer" class="px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/10 text-[0.7rem] hover:opacity-80 transition-opacity">{site.telegram}</a></p>
              <p class="text-xs text-neutral-500 dark:text-neutral-400 leading-relaxed">申请模板，把内容复制修改后通过 Telegram 发送</p>
